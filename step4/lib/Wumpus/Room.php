...
class Room {
    ...
    /**
     * Get a room's contents
     * @return Array of room contents
     */
    ...
    public function contains($item, $recurse=0) {
        ...
        if (in_array($item, $this->contents) and $recurse == 0) {
            $flag = true;
        }

        elseif ($recurse == 1) {
            foreach ($this->neighbors as $element) {
                if (in_array($item, $element->getContent())) {
                    $flag = true;
                }
            }
        }
        ...
        elseif ($recurse == 2) {
            foreach ($this->neighbors as $element1) {
                foreach ($element1->getNeighbors() as $element2) {
                    if (in_array($item, $element2->getContent())) {
                        $flag = true;
                    }
                }
            }
