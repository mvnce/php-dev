...
class Wumpus {
    ...
    public function move($ndx) {
        ...
        elseif (in_array($ndx, array(5, 15)) ) {
            return self::FELL;
        }
        elseif ($ndx == 19) {
            $this->carried = true;
            $this->current = $this->rooms[15];
            return self::BIRDS;
        }
        else {
            return self::HAPPY;
    ...
    public function wasCarried() {
        return $this->carried ? true : false;
    }
...
//        echo $this->current->getNdx();
        if ($this->current->contains(1, 1) or $this->current->contains(1, 2)) {
            return true;
        }
        return false;
    }
    ...
    public function feelDraft() {
        if ($this->current->contains(Wumpus::PIT, 1)) {
            return true;
        }
        else {
            return false;
    ...
    public function hearBirds() {
        if ($this->current->contains(Wumpus::BIRDS, 1)) {
            return true;
        }
        else {
            return false;
